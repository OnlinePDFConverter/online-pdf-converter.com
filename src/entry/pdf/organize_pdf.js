...
import '@css/pdf/organize_pdf.css';
...
const outputFileName = '{name}_organized.pdf';
const { startProcess } = initWorker({
    worker: new Worker(new URL('@src/workers/organize_pdf.worker.js', import.meta.url), { type: 'module' }),
    fileUpload,
    elements,
    outputFileName
});
...
function addBlankPage(pageElement) {
    ...
    blankPage.querySelector('[data-dz-name]').textContent = $L.get('organize_pdf.blankPage');
