...
import '@css/pdf/pdf_to_images.css';
...
const outputFileName = data => {
    ...
    return outputCount > 1
        ? `{name}_pdf_to_images.${data.outputFormat}`
        : `{name}.${data.outputFormat}`;
};
const { startProcess } = initWorker({
    worker: new Worker(new URL('@src/workers/pdf_to_images.worker.js', import.meta.url), { type: 'module' }),
    fileUpload,
    elements,
    outputFileName
});
