...
import '@css/pdf/rasterize_pdf.css';
...
const outputFileName = data => {
    ...
    return outputCount > 1
        ? `{name}_rasterize.${data.outputFormat}`
        : `{name}.${data.outputFormat}`;
};
const { startProcess } = initWorker({
    worker: new Worker(new URL('@src/workers/rasterize_pdf.worker.js', import.meta.url), { type: 'module' }),
    fileUpload,
    elements,
    outputFileName
});
